...
var _ = Describe("FusionAccess Controller Suite", func() {

	var cnsaVersion string

	BeforeEach(func() {
		b, _ := os.ReadFile("../../CNSA_VERSION.txt")
		cnsaVersion = strings.TrimSpace(string(b))
	})
	...
	var _ = Describe("getIbmManifest", func() {
		Context("when ExternalManifestURL is valid", func() {
			It("should return the external URL if allowed", func() {
				...
				_, url, err := getIbmManifest(fusionObj)
				Expect(err).ToNot(HaveOccurred())
			...
			It("should return an error if external URL is disallowed", func() {
				...
				_, _, err := getIbmManifest(fusionObj)
				Expect(err).To(HaveOccurred())
		...
		Context("when IbmCnsaVersion is set and external URL is empty", func() {
			It("should return the install path", func() {
				fusionObj := fusionv1alpha.FusionAccessSpec{
					ExternalManifestURL: "",
					StorageScaleVersion: "v1.2.3.4",
				}

				cnsa, installPath, err := getIbmManifest(fusionObj)
				Expect(err).ToNot(HaveOccurred())
				Expect(cnsa).To(Equal(cnsaVersion))
				Expect(installPath).To(Equal(path.Join("../../files", cnsaVersion, "install.yaml")))
			})
		})
	})
