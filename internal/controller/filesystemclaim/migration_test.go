...
var _ = Describe("Migration Helper Functions", func() {
	...
	Describe("Full Migration Flow", func() {
		...
		It("should not create FSC when all LocalDisks have invalid WWN-based names", func() {
			// All LocalDisks use invalid/legacy/non-WWN names - the whole group should be skipped
			// because isV1LocalDisk() filters them out before grouping
			ld1 := createV1LocalDisk("sda", namespace, "/dev/sda", "worker-1", "test-fs-all-invalid-wwn")
			ld2 := createV1LocalDisk("sdb", namespace, "/dev/sdb", "worker-2", "test-fs-all-invalid-wwn")

			// Filesystem that references the above LocalDisks
			fs := createV1Filesystem("test-fs-all-invalid-wwn", namespace)
			...
			// Run migration - invalid LocalDisks are filtered out by isV1LocalDisk()
			// so no group is created, and no FSC should be created
			err := RunMigration(ctx, fakeClient)
			Expect(err).NotTo(HaveOccurred())

			// Verify no FSC was created because all LocalDisks were filtered out
			fsc := &fusionv1alpha1.FileSystemClaim{}
			err = fakeClient.Get(ctx, client.ObjectKey{
				Name:      "test-fs-all-invalid-wwn",
				Namespace: namespace,
			}, fsc)
			Expect(err).To(HaveOccurred())
			Expect(apierrors.IsNotFound(err)).To(BeTrue(), "FSC should not be created when all LocalDisks have invalid names")

			// Verify invalid LocalDisks do NOT have ownerRef (were skipped)
			for _, ldName := range []string{"sda", "sdb"} {
				updatedLD := createV1LocalDisk("", "", "", "", "")
				err = fakeClient.Get(ctx, client.ObjectKey{
					Name:      ldName,
					Namespace: namespace,
				...
				Expect(err).NotTo(HaveOccurred())
				ownerRefs := updatedLD.GetOwnerReferences()
				Expect(ownerRefs).To(BeEmpty(), "Invalid LocalDisk %s should not have ownerRef", ldName)
			}
		})
